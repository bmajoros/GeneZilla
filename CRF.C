...
#ifdef EXPLICIT_GRAPHS
ParseGraph &CRF::parse(const BOOM::String &fastaFilename,
...
#ifdef EXPLICIT_GRAPHS
ParseGraph &CRF::parse(const BOOM::String &isochoreFilename,
			    const Sequence &substrate,
			    const BOOM::String &substrateString,
			    float &gcContent)
{
...
float CRF::getGCcontent(const BOOM::String &seq)
{
...
void CRF::instantiateLeftTermini()
{
...
BOOM::Stack<SignalPtr> *CRF::instantiateRightTermini(
...
void CRF::linkBack(const BOOM::String &str,SignalPtr newSignal)
{
...
inline void CRF::selectPredecessors(int newConsPos,
					 SignalQueue &queue,
...
void CRF::selectCodingPred(int newConsPos,
				SignalQueue &queue,
...
inline void CRF::selectIntronPred(int newConsPos,SignalQueue &q,
				       Strand strand,double bestScore[3],
				       SignalPtr bestPred[3],
				       ContentType contentType,
				       SignalType toType,
...
inline void CRF::selectIntergenicPred(int newConsPos,
					   SignalQueue &queue,
					   Strand strand,
					   double bestScore[3],
					   SignalPtr bestPred[3],
...
void CRF::enqueue(SignalPtr signal)
{
...
void CRF::handleStopCodons(const BOOM::String &str,int pos)
{
...
void CRF::terminateForwardORFs(int TAGposition)
{
  if(!prohibitPTCs) return;
...
void CRF::terminateReverseORFs(int TAGposition)
{
  if(!prohibitPTCs) return;
...
BOOM::Stack<SignalPtr> *CRF::traceBack(SignalPtr rightTerminus,
					     int phase)
{
...
int CRF::getPhase(SignalPtr signal)
{
...
double CRF::scoreExon(SignalPtr thisSignal,SignalPtr nextSignal,
...
void CRF::generateGff(BOOM::Stack<SignalPtr> *path,int seqLen,
			   double parseScore)
{
	  ...
	  switch(nextType)
	    ...
	    case GT:
	      cout << substrateId<<'\t'<<PROGRAM_NAME<<"\tinitial-exon\t"
		   ...
		   << "transcript_id=" << transcriptId << ";" << endl;
	      break;
	    case TAG:
...
double CRF::scoreIntronPhases(SignalType predType,SignalType toType,
...
void CRF::observeRecentStopCodons(const BOOM::String &str,
				       SignalPtr signal)
{
  if(!prohibitPTCs) return;
  /*
...
int CRF::mapPhaseBack(int phase,SignalPtr right,SignalPtr left)
{
...
DiscreteDistribution &CRF::getDistribution(ContentType t)
{
...
ContentType CRF::exonTypeToContentType(ExonType exonType,
					       Strand strand)
{
...
SignalSensor &CRF::getSignalSensor(SignalType t)
{
...
ContentSensor &CRF::getContentSensor(ContentType t)
{
...
void CRF::computeIntergenicSums(const Sequence &seq,
				     const BOOM::String &str,
				     const char *p)
{
...
#ifdef EXPLICIT_GRAPHS
BOOM::Vector<SignalPtr> *CRF::getPathFromGff(
...
#ifdef FORCE_SPECIFIC_SIGNALS
void CRF::forceSignalCoords(BOOM::Vector<int> &signalCoords)
{
...
void CRF::switchIsochore(float gcContent,int pos)
{
...
void CRF::processIsochoreFile(const BOOM::String &filename,
				   float gcContent)
{
...
void CRF::createQueue(ContentType contentType)
{
...
void CRF::resetAccumulatorPositions()
{
...
void CRF::loadIsochoreBoundaries(const BOOM::String &filename)
{
...
void CRF::crossIsochoreBoundary(int pos)
{
...
void CRF::loadCpGislands(const BOOM::String &filename)
{
...
BOOM::StringMap<char> *CRF::getStopCodonConsensuses()
{
  return 
...
Transitions *CRF::getTransitionProbs()
{
