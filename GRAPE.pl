...
Prerequisites:
  ...
  2) Must have validation set (to avoid overtraining) in ./test.gff
...
unless(`cmp iso$isochore.gff test.gff`) {$skipValidation=1}
...
if(!-e "chunks")
  {
	...
	system("rm -r chunks ; mv new-chunks validation-chunks");
      }
    else {system("mv chunks validation-chunks")}

    system("genezilla/make-test-set.pl iso$isochore.gff iso$isochore.fasta $maxTestSet 1000 TGA,TAA,TAG");
    if(!$opt_C)
      {
	system("genezilla/combine-chunks.pl");
	system("rm -r chunks ; mv new-chunks chunks");
      }
  }
if(!-e "genezilla.top")
...
if(!-e "donors$isochore.fasta")
  {
    ...
    system("genezilla/get-examples.pl iso$isochore.gff iso$isochore.fasta "
           . "TAG,TGA,TAA notrim");
  }
...
unless($skipValidation)
  {
    system("mv chunks train-chunks ; mv validation-chunks chunks");
    $validationBaseline=getBaseline();
    system("mv chunks validation-chunks ; mv train-chunks chunks");
  }
...
sub gradientAscent
  {
	...
	for(my $dim=0 ; $dim<$numDim ; ++$dim)
	  ...
	  {
	    system("mv chunks train-chunks ; mv validation-chunks chunks");
	    my $newValidationScore=evaluatePoint($currentPoint,$state);
	    system("mv chunks validation-chunks ; mv train-chunks chunks");
	    if(compareScores($newValidationScore,$currentValidationScore)<0)
	      {
		print LOG "Validation set accuracy decreased, quitting\n";
		printScore("   old V-score: ",$currentValidationScore);
		printScore("   new V-score: ",$newValidationScore);
		$currentPoint=$safePoint;
		last;
	      }
	    $currentValidationScore=$newValidationScore;
	  }
      }
