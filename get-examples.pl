...
##########################################################################
# outputs examples of all features for the gene finder
##########################################################################
...
my $usage="
$name <*.gff> <*.fasta> TGA,TAG,TAA <trim|notrim>

  where: <trim|notrim> indicates whether GFF exons include UTR
";
die "$usage\n" unless @ARGV==4;
my ($gffFilename,$fastaFilename,$stopCodons,$shouldTrim)=@ARGV;
$shouldTrim="\L$shouldTrim";
die "$usage\n" unless $shouldTrim=~/(trim)|(notrim)/;
$shouldTrim=($shouldTrim eq "trim");
...
$gffFilename=~/iso(.+)-(.+)\.gff$/ || 
  die "can't parse isochore def from gff filename";
my ($isoLowerBound,$isoUpperBound)=($1,$2);
...
# CREATE OUTPUT FILES
open(INITIALEXONS,">initial-exons$iso.fasta");
open(FINALEXONS,">final-exons$iso.fasta");
open(INTERNALEXONS,">internal-exons$iso.fasta");
open(SINGLEEXONS,">single-exons$iso.fasta");
open(DONORS,">donors$iso.fasta");
open(ACCEPTORS,">acceptors$iso.fasta");
open(STARTCODONS,">start-codons$iso.fasta");
open(STOPCODONS,">stop-codons$iso.fasta");
open(INTRONS,">introns$iso.fasta");
open(INTERGENIC,">intergenic$iso.fasta");
open(FIVEPRIMEUTR,">five-prime-UTR$iso.fasta");
open(THREEPRIMEUTR,">three-prime-UTR$iso.fasta");
